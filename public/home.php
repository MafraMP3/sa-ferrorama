<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.8/dist/css/bootstrap.min.css" rel="stylesheet"
        integrity="sha384-sRIl4kxILFvY47J16cr9ZwB07vP4J8+LH7qKQnuqkuIAvNWLzeN8tE5YBujZqJLB" crossorigin="anonymous">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css">
    <link rel="stylesheet" href="../styles/style.css">
    <link rel="icon" href="../assets/images/icon.png">
    <title>Home</title>
</head>

<body>

<!------------------------------------Sidebar---------------------------------------//-->

    <div class="sidebar-overlay" id="overlay" onclick="toggleSidebar()"></div>
    <button class="hamburger" onclick="toggleSidebar()">
        <i class="fa-solid fa-bars"></i>
    </button>

    <div class="sidebar container">
        <div id="logocontainer">
            <img src="../assets/images/logo-quadrada.png " alt="logo">
        </div>

        <a href="#" class="sidebar-link" id="linkDashboard">
            <i class="fa-solid fa-house" style="color: #fff;"></i> Dashboard
        </a>

        <a href="sensores.php" class="sidebar-link" id="linkSensores">
            <i class="fa-solid fa-satellite-dish" style="color: #fff;"></i> Sensores
        </a>

        <a href="monitoramento.php" class="sidebar-link" id="linkMonitoramento">
            <i class="fa-solid fa-chart-line" style="color: #fff;"></i> Monitoramento
        </a>

        <a href="usuarios.php" class="sidebar-link" id="linkUsuarios">
            <i class="fa-solid fa-users" style="color: rgb(255, 255, 255);"></i></i> Usuários
        </a>

        <a href="#" class="logout" id="linkSair">SAIR</a>
    </div>

<!------------------------------------------------------------------------------------//-->

<!---------DADOS--------->


    <div class="content ">

        <h3 id="saudacao">Bem-vindo, (nome admin).</h3>
        <p>Gerencie tudo por aqui</p>


        <div class="container-fluid">

            <div class="row g-4 mb-4">

                <div class="col-12 col-md-6 col-xl-4">
                    <div class="card dashboard-card p-4 h-100">
                        <div class="d-flex align-items-center gap-3">
                            <img src="../assets/images/icon-sensores.png" alt="icone sensores" class="imagem-analise">
                            <div>
                                <p class="h5 mb-1">
                                    Sensores ativos
                                </p>

                                <p class="lead mb-0">
                                    ?? sensores online
                                </p>

                            </div>
                        </div>
                    </div>
                </div>


                <div class="col-12 col-md-6 col-xl-4">
                    <div class="card dashboard-card p-4 h-100">
                        <div class="d-flex align-items-center gap-3">

                            <img src="../assets/images/icon-velocidade.png" alt="imagem velocimetro"
                                class="imagem-analise">
                            <div>
                                <p class="h5 mb-1">
                                    Velocidade atual
                                </p>

                                <p class="lead mb-0">
                                    ?? km/h
                                </p>
                            </div>
                        </div>
                    </div>
                </div>

                <div class="col-12 col-md-6 col-xl-4">
                    <div class="card dashboard-card p-4 h-100">
                        <div class="d-flex align-items-center gap-3">
                            <img src="../assets/images/icon-status.png" alt="imagem status" class="imagem-analise">
                            <div>

                                <p class="h5 mb-1">
                                    Status atual
                                </p>

                                <p class="lead mb-0">
                                    ??
                                </p>

                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <div class="row g-4 dashboard-bottom">
                <div class="col-12 col-xl-8">
                    <div class="card dashboard-card p-4 h-100">
                        <div class="d-flex justify-content-between align-items-center mb-4">
                            <h4>
                                Monitoramento Geral
                            </h4>
                            <i class="fa-solid fa-chart-line text-danger opacity-50 fa-xl"></i>
                        </div>
                        <div class="monitoramento-area">
                            <div class="text-center">
                                <i class="fa-solid fa-train fa-3x mb-3"></i>

                                <h5>
                                    aaaaaaaaaaaaaaaaaaaaaaaaaaaa
                                </h5>

                                <p>
                                    aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa
                                </p>

                            </div>
                        </div>
                    </div>
                </div>


                <div class="col-12 col-xl-4">
                    <div class="d-flex flex-column gap-4 h-100">
                        <div class="card dashboard-card p-4 flex-fill">
                            <div class="d-flex justify-content-between align-items-center mb-3">
                                <h5>
                                    Status do Sistema
                                </h5>
                                <i class="fa-solid fa-circle-check text-danger opacity-50"></i>
                            </div>
                            <div class="status-area">
                                <h2>
                                    ??????
                                </h2>
                                <p>
                                    Aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa
                                </p>

                            </div>

                        </div>
                        <div class="card dashboard-card p-4 flex-fill">
                            <div class="d-flex justify-content-between align-items-center mb-3">

                                <h5>
                                    Alertas
                                </h5>

                                <i class="fa-solid fa-triangle-exclamation text-danger opacity-50"></i>

                            </div>
                            <div class="alerta-area">
                                <h2>
                                    0
                                </h2>


                            </div>
                        </div>
                    </div>
                </div>
            </div>

<!--------------------------------->

        </div>
        <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.8/dist/js/bootstrap.bundle.min.js"
            integrity="sha384-FKyoEForCGlyvwx9Hj09JcYn3nv7wiPVlz7YYwJrWVcXK/BmnVDxM+D2scQbITxI"
            crossorigin="anonymous"></script>
        <script src="../scripts/script.js"></script>
</body>

</html>